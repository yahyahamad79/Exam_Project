...
def extract_questions_from_pdf(text):
    questions = []
    # تقسيم النص بناءً على النقطة أو السطر الجديد أو الفاصلة المنقوطة
    sentences = re.split(r'[.\n؛]', text)
    count = 1
    
    # كلمات مفتاحية تشير إلى محتوى تعليمي هام
    key_words = ["يعرف", "تعد", "تعتبر", "يتكون", "هدف", "أهمية", "ميزة", "خصائص", "انواع"]
    
    for sentence in sentences:
        clean_text = sentence.strip()
        
        # خفضنا الحد الأدنى للحروف إلى 20 لضمان التقاط الجمل العربية
        if len(clean_text) > 20:  
            # التحقق مما إذا كانت الجملة مفيدة أو تحتوي على كلمة مفتاحية
            is_educational = any(word in clean_text for word in key_words)
            
            if is_educational or len(clean_text) > 60:
                level = "بسيط"
                if any(word in clean_text for word in ["بسبب", "بينما", "نتيجة", "اختلاف"]):
                    level = "متوسط"
                if any(word in clean_text for word in ["التحوط", "الاستراتيجي", "تحليل", "رؤية"]):
                    level = "متقدم"
                
                # صياغة السؤال بشكل احترافي
